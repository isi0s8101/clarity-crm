...
const normalizedObject = validateConfiguration("object", {
  key: " Asset ",
  label: "Actif",
  fields: [{ key: "company", label: "Société", type: "relation", targetType: " Company " }],
});
assert.equal(normalizedObject.ok, true);
assert.equal(normalizedObject.value.key, "asset");
assert.equal(normalizedObject.value.fields[0].targetType, "company");
assert.equal(isSafeConfiguredPattern("^[A-Z]{3}-[0-9]{3}$"), true);
assert.equal(isSafeConfiguredPattern("(a+)+$"), false);
assert.equal(isSafeConfiguredPattern("^a*a*$"), false);
assert.equal(
  validateConfiguration("object", {
    key: "vehicle",
    label: "Véhicule",
...
assert.equal(
  validateConfiguration("pipeline", {
    ...
    objectType: "opportunity",
    stages: [{ key: "new", label: "Nouveau" }, { key: "won", label: "Gagné" }],
    transitions: [{ from: "new", to: "missing" }],
  }).ok,
  false,
);
assert.equal(
...
assert.equal(
  ...
  true,
  "les formulaires doivent pouvoir référencer les champs cœur camelCase",
);
assert.equal(
  validateConfiguration("form", {
  ...
  false,
  "les clés techniques de formulaire restent strictement lowercase",
);
assert.equal(
  validateConfiguration("relation", {
    key: "vehicle_company",
    sourceType: "vehicle",
    targetType: "company",
...
assert.equal(
  validateConfiguration("object", {
    key: "asset",
    label: "Actif",
    fields: [{ key: "state", label: "État", type: "select", options: ["new", "new"] }],
  }).ok,
  false,
);
assert.equal(
  validateConfiguration("object", {
    key: "asset",
    label: "Actif",
    fields: [{ key: "serial", label: "Série", type: "text", minLength: 10, maxLength: 2 }],
  }).ok,
  false,
);
assert.equal(
