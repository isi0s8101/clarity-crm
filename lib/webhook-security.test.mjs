...
assert.equal(hostMatchesAllowedWebhookHosts("hooks.example.com", "https://hooks.example.com"), false);
assert.deepEqual((await validateWebhookTargetUrl("https://public.example/hook", {
  allowedHosts: "https://hooks.example.com",
...
const reboundFetch = globalThis.fetch;
try {
  globalThis.fetch = async () => {
    throw new Error("fetch ne doit pas être appelé quand une IP validée est épinglée");
  };
  await assert.rejects(
    ...
    /Timeout webhook dépassé|connect|network|TLS|Client network socket/i,
  );
} finally {
  globalThis.fetch = reboundFetch;
}
