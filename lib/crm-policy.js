...
function validateObjectDefinition(definition) {
  ...
  for (const field of definition.fields) {
    ...
    if (field.type === "relation" && field.targetType !== undefined && !normalizeRecordType(field.targetType)) {
      throw new Error("Objet cible du champ relation invalide.");
    }
    ...
    if (field.pattern !== undefined) {
      if (typeof field.pattern !== "string" || field.pattern.length < 1 || field.pattern.length > 200) {
        throw new Error("Expression de validation invalide.");
      }
      try {
...
function validateRelationDefinition(definition) {
  ...
  if (!normalizeRecordType(definition.sourceType)) throw new Error("Objet source de relation invalide.");
  if (!normalizeRecordType(definition.targetType)) throw new Error("Objet cible de relation invalide.");
