...
const FORM_FIELD_KEY_RE = /^[a-z][A-Za-z0-9_-]{0,49}$/;
...
export function validateConfiguration(kind, definition) {
  ...
  try {
    ...
    if (kind === "relation") validateRelationDefinition(definition);
...
export function isSafeConfiguredPattern(pattern) {
  if (typeof pattern !== "string" || pattern.length < 3 || pattern.length > 120) return false;
  ...
  for (let index = 1; index < pattern.length - 1; index += 1) {
    ...
    if (character === "{") {
      ...
      const match = /^\{(\d{1,4})(?:,(\d{1,4}))?\}/.exec(pattern.slice(index));
      if (!match) return false;
      const minimum = Number(match[1]);
      const maximum = match[2] === undefined ? minimum : Number(match[2]);
      if (minimum > maximum || maximum > 1000) return false;
      index += match[0].length - 1;
      hasAtom = false;
      continue;
    }
    if ("}()*+?|.".includes(character) || character === "^" || character === "$") return false;
    hasAtom = true;
  }
...
function validateObjectDefinition(definition) {
  ...
  for (const field of definition.fields) {
    ...
    if (field.required !== undefined && typeof field.required !== "boolean") {
      throw new Error("Indicateur required invalide.");
    }
    if (field.type === "select") {
      ...
      const options = field.options.map((option) => normalizeText(option, 120));
      if (options.some((option) => !option) || new Set(options).size !== options.length) {
        throw new Error("Options de champ select invalides ou dupliquées.");
      }
    }
    if (field.type === "relation" && field.targetType !== undefined) {
      const targetType = normalizeRecordType(field.targetType);
      if (!targetType) throw new Error("Objet cible du champ relation invalide.");
      field.targetType = targetType;
    }
    if (field.min !== undefined && (typeof field.min !== "number" || !Number.isFinite(field.min))) {
      throw new Error("Borne minimale de champ invalide.");
    }
    if (field.max !== undefined && (typeof field.max !== "number" || !Number.isFinite(field.max))) {
      throw new Error("Borne maximale de champ invalide.");
    }
    if (typeof field.min === "number" && typeof field.max === "number" && field.min > field.max) {
      throw new Error("Bornes de champ incohérentes.");
    }
    for (const property of ["minLength", "maxLength"]) {
      if (field[property] !== undefined && (!Number.isInteger(field[property]) || field[property] < 0 || field[property] > 10000)) {
        throw new Error("Longueur de champ invalide.");
      }
    }
    if (typeof field.minLength === "number" && typeof field.maxLength === "number" && field.minLength > field.maxLength) {
      throw new Error("Longueurs de champ incohérentes.");
    }
    if (field.pattern !== undefined) {
      if (!isSafeConfiguredPattern(field.pattern)) throw new Error("Expression de validation invalide ou non sûre.");
    }
  }
}
...
function validatePipelineDefinition(definition) {
  ...
  if (definition.transitions !== undefined) {
    if (!Array.isArray(definition.transitions) || definition.transitions.length > 300) {
      throw new Error("Transitions de pipeline invalides.");
    }
    const transitions = new Set();
    for (const transition of definition.transitions) {
      if (!isPlainObject(transition) || !seen.has(transition.from) || !seen.has(transition.to) || transition.from === transition.to) {
        throw new Error("Transition de pipeline invalide.");
      }
      const key = `${transition.from}:${transition.to}`;
      if (transitions.has(key)) throw new Error("Transition de pipeline dupliquée.");
      transitions.add(key);
    }
  }
}
...
function validateFormDefinition(definition) {
  ...
  const objectType = normalizeRecordType(definition.objectType);
  if (!objectType) throw new Error("Type d'objet du formulaire invalide.");
  definition.objectType = objectType;
  ...
  const seen = new Set();
  for (const field of definition.fields) {
    if (!isPlainObject(field) || !FORM_FIELD_KEY_RE.test(String(field.key ?? ""))) throw new Error("Champ de formulaire invalide.");
    if (seen.has(field.key)) throw new Error("Champ de formulaire dupliqué.");
    seen.add(field.key);
    if (field.required !== undefined && typeof field.required !== "boolean") {
      throw new Error("Indicateur required de formulaire invalide.");
    }
  }
}

function validateRelationDefinition(definition) {
  if (!KEY_RE.test(String(definition.key ?? ""))) throw new Error("Clé de relation invalide.");
  const sourceType = normalizeRecordType(definition.sourceType);
  const targetType = normalizeRecordType(definition.targetType);
  if (!sourceType) throw new Error("Objet source de relation invalide.");
  if (!targetType) throw new Error("Objet cible de relation invalide.");
  definition.sourceType = sourceType;
  definition.targetType = targetType;
  if (!["one_to_one", "one_to_many", "many_to_one", "many_to_many"].includes(definition.cardinality)) {
    throw new Error("Cardinalité de relation invalide.");
  }
}
...
function validateAutomationDefinition(definition) {
  ...
  for (const action of definition.actions) {
    ...
    if (action.kind === "timeline" && !normalizeText(action.summary, 240)) {
      throw new Error("Résumé d'automatisation requis.");
    }
    if (action.kind === "update_field" && !KEY_RE.test(String(action.field ?? ""))) {
      throw new Error("Champ d'automatisation invalide.");
    }
    if (action.kind === "webhook" && !normalizeWebhookUrl(action.url)) {
      throw new Error("URL webhook d'automatisation invalide.");
    }
    if (action.kind === "notify_owner" && !normalizeText(action.message, 240)) {
      throw new Error("Message de notification requis.");
    }
    if (action.kind === "timeline" && !normalizeText(action.summary, 240)) {
