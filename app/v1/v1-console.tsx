...
const CONFIG_EXAMPLES: Record<string, Record<string, unknown>> = {
  ...
  relation: { key: "contact_company", sourceType: "contact", targetType: "company", cardinality: "many_to_one" },
...
export function V1Console({ user }: { user: { email: string; displayName: string } }) {
  ...
  const submitConfiguredForm = async (event: FormEvent) => {
    ...
    setBusy(true);
    try {
      const values = Object.fromEntries(selectedFormFields.map((field, index) => {
        const key = typeof field.key === "string" ? field.key : `field_${index + 1}`;
        const raw = formFieldValues[key];
        if (raw === undefined || raw === "") return [key, raw];
        if (field.type === "number" || field.type === "currency") return [key, Number(raw)];
