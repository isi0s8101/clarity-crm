...
export function V1Console({ user }: { user: { email: string; displayName: string } }) {
  ...
  const uploadDocument = async (file: File) => {
    ...
    setBusy(true);
    try {
      const form = new FormData(); form.set("recordId", selected.id); form.set("file", file); form.set("category", documentMetadata.category); form.set("tags", documentMetadata.tags); form.set("description", documentMetadata.description);
      const payload = await request("/api/documents", { method: "POST", body: form });
      setDocuments((current) => [payload.item as DocumentItem, ...current]);
      setDocumentMetadata({ category: "", tags: "", description: "" });
      setMessage("Document téléversé et lié à la fiche.");
    } catch (error) { setMessage(errorMessage(error)); } finally { setBusy(false); }
  };

  const loadDocumentVersions = async (id: string) => {
    try {
      const payload = await request(`/api/documents/${encodeURIComponent(id)}/versions`);
      setDocumentVersions((current) => ({ ...current, [id]: Array.isArray(payload.items) ? payload.items as DocumentVersionItem[] : [] }));
    } catch (error) { setMessage(errorMessage(error)); }
  };

  const uploadDocumentVersion = async (id: string, file: File) => {
    setBusy(true);
    try {
      const form = new FormData(); form.set("file", file);
      const payload = await request(`/api/documents/${encodeURIComponent(id)}/versions`, { method: "POST", body: form });
      const item = payload.item as DocumentVersionItem;
      setDocumentVersions((current) => ({ ...current, [id]: [item, ...(current[id] ?? [])] }));
      setDocuments((current) => current.map((document) => document.id === id ? { ...document, originalName: item.originalName, normalizedName: item.normalizedName, mimeType: item.mimeType, sizeBytes: item.sizeBytes, currentVersion: item.version } : document));
      setMessage(`Version ${item.version} du document créée.`);
    } catch (error) { setMessage(errorMessage(error)); } finally { setBusy(false); }
  };
        ...
        <section className="min-w-0 space-y-5">
          ...
          {view === "crm" ? (
            <>
              ...
              {selected ? <section className="grid gap-5 xl:grid-cols-[minmax(260px,.8fr)_minmax(320px,1.35fr)_minmax(240px,.7fr)]"><div className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[.12em] text-blue-700">{TYPES.find(([key]) => key === selected.type)?.[1] ?? selected.type}</p><h2 className="mt-1 truncate text-lg font-semibold">{selected.title}</h2><p className="mt-1 text-sm text-slate-500">{selected.status}</p></div><div className="flex flex-col gap-2"><button disabled={busy} onClick={() => void toggleFavorite()} className="rounded-lg border px-3 py-2 text-sm">{favorites.some((item) => item.resourceType === "crm_record" && item.resourceId === selected.id) ? "Retirer des favoris" : "Ajouter aux favoris"}</button><button disabled={busy} onClick={() => void archiveSelected()} className="rounded-lg border border-red-200 px-3 py-2 text-sm text-red-700">Archiver</button></div></div><dl className="mt-5 space-y-3 text-sm">{Object.entries(selected.data).slice(0, 5).map(([key, value]) => <div key={key} className="border-b border-slate-100 pb-3"><dt className="text-xs font-medium text-slate-500">{key}</dt><dd className="mt-1 break-words text-slate-900">{typeof value === "string" || typeof value === "number" || typeof value === "boolean" ? String(value) : "Valeur structurée"}</dd></div>)}</dl><details className="mt-4 rounded-xl border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-medium">Afficher les données avancées</summary><pre className="mt-3 max-h-64 overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-slate-100">{pretty(selected.data)}</pre></details></div><div className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-semibold">Activité</h2><p className="mt-1 text-sm text-slate-500">Événements rattachés à cet enregistrement.</p><div className="mt-4 divide-y divide-slate-100">{timeline.length ? timeline.map((item) => <div key={item.id} className="py-4"><strong className="text-sm">{item.summary}</strong><p className="mt-1 text-xs text-slate-500">{item.eventType} · {new Date(item.createdAt).toLocaleString("fr-FR")}</p></div>) : <p className="rounded-xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">Aucune activité visible pour le moment.</p>}</div></div><aside className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-semibold">Documents liés</h2><p className="mt-1 text-xs text-slate-500">Stockage POSIX isolé, versions immuables.</p><input disabled={busy} className="mt-3 block w-full text-xs" type="file" accept=".pdf,.txt,.csv,.jpg,.jpeg,.png,.docx,.xlsx" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadDocument(file); event.currentTarget.value = ""; }} /><div className="mt-3 grid gap-2"><input maxLength={80} className="rounded border px-2 py-1 text-xs" value={documentMetadata.category} onChange={(event) => setDocumentMetadata({ ...documentMetadata, category: event.target.value })} placeholder="Catégorie" /><input maxLength={800} className="rounded border px-2 py-1 text-xs" value={documentMetadata.tags} onChange={(event) => setDocumentMetadata({ ...documentMetadata, tags: event.target.value })} placeholder="Tags séparés par virgule" /><textarea maxLength={2000} rows={2} className="rounded border px-2 py-1 text-xs" value={documentMetadata.description} onChange={(event) => setDocumentMetadata({ ...documentMetadata, description: event.target.value })} placeholder="Description" /></div><div className="mt-3 space-y-2">{documents.length ? documents.map((document) => <details key={document.id} className="rounded border p-2" onToggle={(event) => { if ((event.currentTarget as HTMLDetailsElement).open && !documentVersions[document.id]) void loadDocumentVersions(document.id); }}><summary className="cursor-pointer text-xs"><strong>{document.originalName}</strong> · v{document.currentVersion} · {Math.ceil(document.sizeBytes / 1024)} Ko</summary><div className="mt-2 flex gap-2"><a className="min-w-0 flex-1 rounded border p-2 text-xs hover:bg-slate-50" href={`/api/documents/download?id=${encodeURIComponent(document.id)}`}>Télécharger la version courante</a><button disabled={busy} onClick={() => void archiveDocument(document.id)} className="rounded border px-2 text-xs text-red-700">Archiver</button></div><input disabled={busy} className="mt-2 block w-full text-xs" type="file" accept=".pdf,.txt,.csv,.jpg,.jpeg,.png,.docx,.xlsx" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadDocumentVersion(document.id, file); event.currentTarget.value = ""; }} /><div className="mt-2 space-y-1">{(documentVersions[document.id] ?? []).map((version) => <a key={version.id} className="block text-xs text-blue-700" href={`/api/documents/download?id=${encodeURIComponent(document.id)}&version=${version.version}`}>v{version.version} · {version.originalName} · {new Date(version.createdAt).toLocaleString("fr-FR")}</a>)}</div></details>) : <p className="text-xs text-slate-500">Aucun document actif.</p>}</div></aside></section> : null}
