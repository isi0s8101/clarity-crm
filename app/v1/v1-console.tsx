...
import { ConfigurationAdminStudio } from "@/components/configuration-admin-studio";
...
const CONFIG_EXAMPLES: Record<string, Record<string, unknown>> = {
  object: {
    ...
    label: "Équipement",
    fields: [
      { key: "serial", label: "N° de série", type: "text", required: true },
    ],
  },
  pipeline: {
    key: "sales_custom",
  ...
  form: {
    key: "lead_form",
    objectType: "lead",
    fields: [{ key: "title", required: true }, { key: "source" }],
  },
  relation: {
    key: "contact_company",
    sourceType: "contact",
...
export function V1Console({
  ...
  const [configHistory, setConfigHistory] = useState<
    Record<string, ConfigurationVersion[]>
  >({});
  const [configForm, setConfigForm] = useState({
    kind: "object",
    name: "Nouvelle configuration",
    definition: pretty(CONFIG_EXAMPLES.object),
  });
  ...
  const selectedForm = forms.find(
    (item) => String(item.definition.key ?? "") === selectedFormKey,
  );
  ...
  const loadRecords = useCallback(async () => {
    try {
      ...
      setRecords(
        Array.isArray(payload.items) ? (payload.items as RecordItem[]) : [],
      );
    } catch (error) {
      setRecords([]);
  ...
  const loadConfigurations = useCallback(async () => {
    try {
      const payload = await request(
        `/api/configurations${isAdmin ? "?all=1" : ""}`,
      );
      setConfigs(
        Array.isArray(payload.items)
          ? (payload.items as ConfigurationItem[])
          : [],
      );
    } catch (error) {
      setMessage(errorMessage(error));
      ...
      request("/api/configurations?kind=form")
        .then((payload) => {
          const next = Array.isArray(payload.items)
            ...
            : [];
          setForms(next);
          if (!selectedFormKey && next[0])
            setSelectedFormKey(String(next[0].definition.key ?? ""));
        })
        .catch((error) => setMessage(errorMessage(error)));
    }
    if (view === "automations") {
        ...
        .then((payload) => {
          setAutomationRuns(
            Array.isArray(payload.runs)
              ? (payload.runs as Array<Record<string, unknown>>)
              : [],
          );
          setAutomationConfigs(
            Array.isArray(payload.configurations)
              ? (payload.configurations as ConfigurationItem[])
              : [],
          );
        })
        .catch((error) => setMessage(errorMessage(error)));
        ...
        .then((payload) =>
          setWebhookDeliveries(
            Array.isArray(payload.deliveries)
              ? (payload.deliveries as Array<Record<string, unknown>>)
              : [],
          ),
        )
        .catch((error) => setMessage(errorMessage(error)));
      ...
      request("/api/notifications")
        .then((payload) => {
          ...
          setUnreadNotifications(
            typeof payload.unread === "number" ? payload.unread : 0,
          );
        })
        .catch((error) => setMessage(errorMessage(error)));
    }
  ...
  const openRecord = async (item: RecordItem) => {
    ...
    setMessage("");
    try {
      const [recordPayload, timelinePayload, documentPayload, relationPayload] =
        await Promise.all([
      ...
      setEditForm({
        ...
        data: pretty(current.data),
      });
      setRelationForm({ toId: "", relationType: "related_to" });
      setTimeline(
        Array.isArray(timelinePayload.items)
          ? (timelinePayload.items as TimelineItem[])
          : [],
      );
      setDocuments(
        Array.isArray(documentPayload.items)
          ? (documentPayload.items as DocumentItem[])
          : [],
      );
      setRelations(
        Array.isArray(relationPayload.items)
          ? (relationPayload.items as RelationItem[])
          : [],
      );
    } catch (error) {
      setSelected(null);
  ...
  const deleteRelation = async (id: string) => {
    ...
    setMessage("");
    try {
      await request(`/api/crm/relations?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
      setRelations((current) =>
        current.filter((item) => item.relation.id !== id),
      );
  ...
  const searchGlobally = async (event: FormEvent) => {
    ...
    try {
      const payload = await request(
        `/api/crm/search?q=${encodeURIComponent(q)}&limit=20`,
      );
      setGlobalResults(
        Array.isArray(payload.items) ? (payload.items as RecordItem[]) : [],
      );
  ...
  const previewImport = async (confirm: boolean) => {
    ...
    try {
      ...
      const payload = await request("/api/crm/import", {
        method: "POST",
        body: form,
      });
      setImportPreview(payload);
      if (confirm) {
        setMessage("Import exécuté : consultez le rapport ci-dessous.");
        await loadRecords();
      }
    } catch (error) {
      setMessage(errorMessage(error));
  ...
  const archiveSelected = async () => {
    ...
    setBusy(true);
    try {
      await request(`/api/crm?id=${encodeURIComponent(selected.id)}`, {
        method: "DELETE",
      });
      setSelected(null);
      setTimeline([]);
  ...
  const loadHistory = async (id: string) => {
    try {
      const payload = await request(
        `/api/configurations?historyId=${encodeURIComponent(id)}`,
      );
      setConfigHistory((current) => ({
        ...current,
        [id]: Array.isArray(payload.items)
          ? (payload.items as ConfigurationVersion[])
          : [],
      }));
    } catch (error) {
      setMessage(errorMessage(error));
  ...
  const restoreVersion = async (id: string, version: number) => {
    ...
    try {
      await request("/api/configurations", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id, restoreVersion: version }),
      });
  ...
  const toggleAutomation = async (item: ConfigurationItem) => {
    ...
    try {
      ...
      setMessage(
        item.active ? "Automatisation désactivée." : "Automatisation activée.",
      );
    ...
    try {
      const values = Object.fromEntries(
        selectedFormFields.map((field, index) => {
          const key =
            typeof field.key === "string" ? field.key : `field_${index + 1}`;
          const raw = formFieldValues[key];
          if (raw === undefined || raw === "") return [key, raw];
      ...
      setMessage(
        `Formulaire soumis : ${String((payload.item as RecordItem | undefined)?.title ?? "OK")}`,
      );
      setFormFieldValues({});
    } catch (error) {
      setMessage(errorMessage(error));
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <header className="border-b border-slate-200 bg-white px-5 py-4 lg:px-8">
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-xl font-semibold">Espace métier réel</h1>
          </div>
          <div className="flex items-center gap-3">
            <HelpButton
              ...
              view={view}
              recordType={view === "crm" ? type : undefined}
              action={selected ? "edit" : undefined}
            />
            <div className="text-right text-sm">
              <strong>{user.displayName}</strong>
              <p className="text-slate-500">
                {session ? `${session.role} · ${session.tenantId}` : user.email}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto grid max-w-[1600px] gap-5 p-5 lg:grid-cols-[220px_1fr] lg:p-8">
        <aside className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          <nav className="space-y-1">
            {(
              ...
              <button
                key={item}
                className={`w-full rounded-xl px-3 py-2 text-left text-sm font-medium ${view === item ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100"}`}
          ...
          <div className="mt-5 border-t border-slate-100 pt-4 text-xs text-slate-500">
            Données PostgreSQL persistantes · RBAC serveur · isolation tenant
          </div>
        </aside>
        ...
        <section className="min-w-0 space-y-5">
          ...
          {view === "dashboard" ? (
            <section className="space-y-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold">Dashboard réel</h2>
                  <p className="text-sm text-slate-500">
                    Indicateurs calculés uniquement à partir des données
                    accessibles dans le tenant courant.
                  </p>
                </div>
                <button
                ...
                <>
                  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    ...
                    <Metric
                      ...
                      value={formatCents(dashboard.pipelineAmountCents)}
                    />
                    <Metric
                      title="Opportunités gagnées"
                      value={String(dashboard.wonOpportunities)}
                    />
                    <Metric
                      title="Tâches ouvertes"
                      value={String(dashboard.openTasks)}
                    />
                    <Metric
                      title="Tâches échues"
                    ...
                    />
                    <Metric
                      title="Étapes visibles"
                      value={String(Object.keys(dashboard.stageCounts).length)}
                    />
                  </div>
                  <div className="grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
                    <div className="rounded-2xl border border-slate-200 bg-white p-5">
                      <h3 className="font-semibold">Répartition du pipeline</h3>
                      <div className="mt-4 space-y-3">
                        {Object.keys(dashboard.stageCounts).length ? (
                          Object.entries(dashboard.stageCounts).map(
                            ([stage, count]) => (
                              <div
          ...
          {view === "crm" ? (
            <>
                      ...
                      >
                        <span>
                          ...
                          <span className="ml-2 text-xs text-slate-500">
                            {TYPES.find(([key]) => key === item.type)?.[1] ??
                              item.type}{" "}
                            · {item.status}
                          </span>
                        </span>
                ...
                <div className="flex flex-wrap gap-2">
                  {TYPES.map(([key, label]) => (
                    <button
                      ...
                      className={`rounded-lg px-3 py-2 text-sm ${type === key ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700"}`}
                    >
                      {label}
                    </button>
                  ))}
              ...
              <div className="grid gap-5 xl:grid-cols-[1fr_420px]">
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <div>
                      <h2 className="font-semibold">{typeLabel}</h2>
                      <p className="text-sm text-slate-500">
                        {records.length} enregistrement(s) visible(s) selon
                        votre scope.
                      </p>
                    </div>
                    <input
                    ...
                    {records.length === 0 ? (
                      ...
                      records.map((item) => (
                        ...
                        >
                          ...
                          <code className="max-w-40 truncate text-[11px] text-slate-400">
                            {item.id}
                          </code>
                        </button>
                      ))
                ...
                >
                  <h2 className="font-semibold">Créer · {typeLabel}</h2>
                  <p className="mb-4 text-sm text-slate-500">
                    Le tenant, l’équipe et le propriétaire sont imposés côté
                    serveur.
                  </p>
              ...
              <div className="grid gap-5 xl:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                  <h2 className="font-semibold">Import CSV contrôlé</h2>
                  <p className="mt-1 text-sm text-slate-500">
                    Aperçu obligatoire avant écriture · UTF-8 · doublons et
                    validations serveur.
                  </p>
                  <input
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      ...
                      onClick={() => void previewImport(false)}
                      className="rounded-lg border px-3 py-2 text-sm disabled:opacity-50"
                    >
                      Prévisualiser
                    </button>
                    <button
                      disabled={busy || !importPreview || !importFile}
                  ...
                  {importPreview ? (
                    <pre className="mt-4 max-h-48 overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-slate-100">
                      {pretty(importPreview)}
                    </pre>
                  ) : null}
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                  <h2 className="font-semibold">Export contrôlé</h2>
                  <p className="mt-1 text-sm text-slate-500">
                    Périmètre RBAC/scopes, limite de volume et protection contre
                    les formules.
                  </p>
                  <div className="mt-4 flex gap-2">
                    <a
                      className="rounded-lg border px-3 py-2 text-sm"
                      href={`/api/crm/export?type=${encodeURIComponent(type)}`}
                    >
                      CSV UTF-8
                    </a>
                    <a
                      className="rounded-lg border px-3 py-2 text-sm"
                      href={`/api/crm/export?type=${encodeURIComponent(type)}&format=xlsx`}
                    >
                      Excel (.xlsx)
                    </a>
                  </div>
              ...
              {selected ? (
                <section className="grid gap-5 xl:grid-cols-[minmax(260px,.8fr)_minmax(320px,1.35fr)_minmax(240px,.7fr)]">
                  <div className="rounded-2xl border border-slate-200 bg-white p-5">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-xs font-semibold uppercase tracking-[.12em] text-blue-700">
                          {TYPES.find(([key]) => key === selected.type)?.[1] ??
                            selected.type}
                        </p>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-5">
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Événements rattachés à cet enregistrement.
                    </p>
                    <div className="mt-4 divide-y divide-slate-100">
                      {timeline.length ? (
                        timeline.map((item) => (
                          <div key={item.id} className="py-4">
                            <strong className="text-sm">{item.summary}</strong>
                            <p className="mt-1 text-xs text-slate-500">
                              {item.eventType} ·{" "}
                  ...
                  <aside className="rounded-2xl border border-slate-200 bg-white p-5">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Stockage POSIX protégé.
                    </p>
                    <input
                      disabled={busy}
                      className="mt-3 block w-full text-xs"
                      type="file"
                      accept=".pdf,.txt,.csv,.jpg,.jpeg,.png,.docx,.xlsx"
                      onChange={(event) => {
                    ...
                    <div className="mt-3 space-y-2">
                      {documents.length ? (
                        documents.map((document) => (
                          <div className="flex gap-2" key={document.id}>
                            <a
                              className="block min-w-0 flex-1 rounded border p-2 text-xs hover:bg-slate-50"
                              href={`/api/documents/download?id=${encodeURIComponent(document.id)}`}
                            >
              ...
              {selected ? (
                <section className="grid gap-5 xl:grid-cols-2">
                  ...
                  >
                    <h2 className="font-semibold">Modifier cette fiche</h2>
                    <p className="mt-1 text-sm text-slate-500">
                      La modification utilise le PATCH CRM, avec validation,
                      scope et audit côté serveur.
                    </p>
                    ...
                    <label className="mt-3 block text-sm">
                      ...
                      <textarea
                        ...
                        value={editForm.data}
                        onChange={(event) =>
                          setEditForm({ ...editForm, data: event.target.value })
                        }
                      />
                    </label>
                    <button
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-5">
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Les contrôles tenant, RBAC et scopes sont appliqués par
                      l’API.
                    </p>
                    <form
                    ...
                    >
                      <input
                        required
                        className="rounded-lg border border-slate-200 px-3 py-2 text-sm"
                        placeholder="ID de la fiche liée"
                      ...
                      {relations.length ? (
                        relations.map((item) => (
                          ...
                          >
                            <button
                              type="button"
                              onClick={() => void openRecord(item.related)}
                              className="min-w-0 text-left hover:text-blue-700"
                            >
                              <strong className="block truncate text-sm">
                                {item.related.title}
                              </strong>
                            ...
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                void deleteRelation(item.relation.id)
                              }
            ...
            <div className="grid gap-5 xl:grid-cols-[1fr_420px]">
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <h2 className="font-semibold">Studio de configuration</h2>
                    <p className="text-sm text-slate-500">
                      Objets, champs, pipelines, formulaires, relations et
                      automatisations, avec version active visible.
                    </p>
                  </div>
                  <button
                ...
                {isAdmin ? (
                  <div className="mb-5 flex flex-wrap gap-2">
                    {["services", "appointments", "field-service"].map(
                      (key) => (
                        <button
                          disabled={busy}
                          key={key}
                ...
                <div className="divide-y divide-slate-100">
                  {configs.length === 0 ? (
                    <p className="py-8 text-sm text-slate-500">
                      Aucune configuration active dans votre périmètre.
                    </p>
                  ) : (
                    configs.map((item) => (
                      <div key={item.id} className="py-4">
                        <div className="flex flex-wrap justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-xs text-slate-500">
                              {item.kind} · clé{" "}
                              {String(item.definition.key ?? item.id)}
                            </p>
                          </div>
                          <span
                            className={`h-fit rounded-full px-2 py-1 text-xs ${item.active ? "bg-green-50 text-green-700" : "bg-slate-100 text-slate-500"}`}
                          >
                            v{item.version} ·{" "}
                            {item.active ? "Actif" : "Inactif"}
                          </span>
                        </div>
                        <details className="mt-3 rounded-lg border border-slate-100 p-3">
                          <summary className="cursor-pointer text-xs font-medium">
                            Afficher la définition
                          </summary>
                          <pre className="mt-2 max-h-48 overflow-auto rounded bg-slate-950 p-3 text-[11px] text-slate-100">
                            {pretty(item.definition)}
                          </pre>
                        </details>
                        {isAdmin ? (
                          <details
                            className="mt-2 rounded-lg border border-slate-100 p-3"
                            onToggle={(event) => {
                              if (
                                (event.currentTarget as HTMLDetailsElement)
                                  .open &&
                                !configHistory[item.id]
                              )
                          ...
                          >
                            ...
                            <div className="mt-2 space-y-2">
                              {(configHistory[item.id] ?? []).map((version) => (
                                <div
                                  key={version.version}
              ...
              {isAdmin ? (
                <details className="rounded-2xl border border-slate-200 bg-white p-5">
                  <summary className="cursor-pointer text-sm font-medium">Mode expert / diagnostic JSON</summary>
                  <form onSubmit={createConfiguration} className="mt-4">
                  <h2 className="font-semibold">Nouvelle configuration</h2>
                  <p className="mt-1 text-sm text-slate-500">
                    Les champs avancés restent disponibles sans surcharger le
                    parcours principal.
                  </p>
                  <label className="mt-4 block text-sm">
                    Type
                    <select
                      ...
                      onChange={(event) =>
                        setConfigForm({
                          ...
                          name: "Nouvelle configuration",
                          definition: pretty(
                            CONFIG_EXAMPLES[event.target.value] ?? {},
                          ),
                        })
                      }
                    >
                      {Object.keys(CONFIG_EXAMPLES).map((key) => (
                  ...
                  >
                    ...
                    <p className="mt-2 text-xs text-slate-500">
                      La validation complète et les permissions restent côté
                      serveur.
                    </p>
                  </details>
                  <button
                ...
                <div className="rounded-2xl border bg-white p-5 text-sm text-slate-500">
                  Lecture seule pour le profil utilisateur. Les actions
                  d’administration sont contrôlées par le serveur.
                </div>
              )}
            </div>
          ) : null}
          ...
          {view === "config" && isAdmin ? (
            <section className="mt-5"><ConfigurationAdminStudio items={configs} onSaved={() => void loadConfigurations()} /></section>
          ) : null}
          {view === "forms" ? (
            <div className="grid gap-5 xl:grid-cols-[280px_1fr]">
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <h2 className="font-semibold">Formulaires actifs</h2>
                <p className="mt-1 text-sm text-slate-500">
                  Choisissez un parcours configuré par votre équipe.
                </p>
                  ...
                  {forms.length === 0 ? (
                    ...
                    forms.map((item) => (
                      <button
                        key={item.id}
                        onClick={() => {
                          const key = String(item.definition.key ?? "");
                          setSelectedFormKey(key);
                          setFormFieldValues({});
                        }}
                        className={`w-full rounded-lg py-3 text-left ${selectedFormKey === String(item.definition.key ?? "") ? "bg-blue-50 px-3" : ""}`}
                      >
                        <strong className="text-sm">{item.name}</strong>
                        <p className="text-xs text-slate-500">
                          {String(item.definition.key ?? "")} ·{" "}
                          {Array.isArray(item.definition.fields)
              ...
              >
                <h2 className="font-semibold">
                  {selectedForm?.name ?? "Soumettre un formulaire"}
                </h2>
                <p className="mt-1 text-sm text-slate-500">
                  Les champs requis sont indiqués. La validation métier reste
                  côté serveur.
                </p>
                {selectedForm ? (
                  ...
                  <div className="mt-5 rounded-lg border border-dashed p-4 text-sm text-slate-500">
                    Sélectionnez un formulaire pour afficher ses champs.
                  </div>
                )}
                <button
                  disabled={busy || !selectedFormKey}
                  className="mt-5 w-full rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white"
                >
                  Valider et créer
                </button>
              </form>
          ...
          {view === "automations" ? (
            <div className="grid gap-5 xl:grid-cols-[1fr_420px]">
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Visualisation des workflows existants, sans créer de
                      second moteur.
                    </p>
                  </div>
                  <button
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              {String(item.definition.key ?? item.id)} · v
                              {item.version}
                            </p>
                          </div>
                          {isAdmin ? (
                            <button
                              ...
                              className={`rounded-lg px-3 py-2 text-xs font-medium ${item.active ? "bg-green-50 text-green-700" : "bg-slate-100 text-slate-600"}`}
                            >
                              {item.active
                                ? "Actif · désactiver"
                                : "Inactif · activer"}
                        ...
                        <div className="mt-4 grid gap-3 md:grid-cols-3">
                          <WorkflowStep
                            title="Déclencheur"
                            value={formatWorkflowValue(item.definition.trigger)}
                          />
                          <WorkflowStep
                            title="Conditions"
                            value={formatWorkflowValue(
                              item.definition.conditions,
                            )}
                          />
                          <WorkflowStep
                            title="Actions"
                            value={formatWorkflowValue(item.definition.actions)}
                          />
                        </div>
              ...
              <aside className="rounded-2xl border border-slate-200 bg-white p-5">
                <h2 className="font-semibold">Exécutions réelles</h2>
                <p className="mb-4 mt-1 text-sm text-slate-500">
                  Journal auditable, limité au tenant courant.
                </p>
                <pre className="max-h-[520px] overflow-auto rounded-xl bg-slate-950 p-4 text-xs text-slate-100">
                  {pretty(automationRuns)}
                </pre>
              </aside>
            </div>
          ) : null}
          {view === "notifications" ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              <h2 className="font-semibold">Notifications internes</h2>
              <p className="mt-1 text-sm text-slate-500">
                {unreadNotifications} non lue(s), persistées par tenant et
                destinataire.
              </p>
                  ...
                  notifications.map((item) => (
                    ...
                    >
                      <div>
                        ...
                        {item.resourceId ? (
                          <button
                            onClick={() => void openNotificationResource(item)}
          ...
          {view === "webhooks" ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              <h2 className="font-semibold">Livraisons webhook</h2>
              <p className="mb-4 text-sm text-slate-500">
                HMAC-SHA256, journalisation et politique SSRF côté serveur.
              </p>
              {isAdmin ? (
                <pre className="max-h-[620px] overflow-auto rounded-xl bg-slate-950 p-4 text-xs text-slate-100">
                  {pretty(webhookDeliveries)}
                </pre>
              ) : (
                <p className="text-sm text-slate-500">
...
function formatWorkflowValue(value: unknown) {
  if (Array.isArray(value))
    return value.length ? `${value.length} élément(s)` : "Aucune";
