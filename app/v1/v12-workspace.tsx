...
function PlanningPanel({ session, request, run, busy, notify }: Shared & { session: Session | null }) {
  ...
  const loadAvailability = async () => {
    ...
    if (!payload) return;
    const found = Array.isArray(payload.slots) ? payload.slots as Slot[] : [];
    setSlots(found); notify(`${found.length} créneau(x) disponible(s).`);
  };
  const loadReservations = async () => {
    const payload = await run(async () => request(`/api/planning?mode=reservations&${queryBase()}`));
    if (payload) setReservations(Array.isArray(payload.items) ? payload.items as Array<Record<string, unknown>> : []);
  };
  ...
  return <div className={styles.grid}>
    <article className={styles.card}><h3>Disponibilités et créneaux</h3><div className={styles.form}>
    ...
    </div><div className={styles.list}>{slots.slice(0, 40).map((slot) => <div className={styles.item} key={slot.startsAt}><strong>{formatDate(slot.startsAt)} → {formatTime(slot.endsAt)}</strong><div className={styles.meta}>{slot.timezone}</div><button className={styles.button} disabled={busy || !title.trim()} onClick={() => void book(slot)}>Réserver ce créneau</button></div>)}{!slots.length ? <p className={styles.meta}>Aucun créneau chargé. Une règle de disponibilité active est nécessaire.</p> : null}</div></article>
    <article className={styles.card}><h3>Rendez-vous réservés</h3><div className={styles.list}>{reservations.map((item) => <div className={styles.item} key={String(item.id)}><strong>{String(item.title ?? "Rendez-vous")}</strong><div className={styles.meta}>{formatDate(String(item.startsAt))} → {formatTime(String(item.endsAt))}</div><div className={styles.meta}>Ressource {String(item.resourceKind)} · {String(item.status)}</div></div>)}</div></article>
  </div>;
}
...
function PublicPanel({ request, run, busy, notify }: Shared) {
  ...
  const load = async () => { const payload = await run(async () => request("/api/publications")); if (payload) setItems(Array.isArray(payload.items) ? payload.items as Publication[] : []); };
  const create = async (event: FormEvent) => {
    event.preventDefault();
    const exposedFields = fields.split(",").map((value) => value.trim()).filter(Boolean);
    const payload = await run(async () => request("/api/publications", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ formKey, kind, exposedFields, policy: { rateLimitPerHour: 20 } }) }));
    if (!payload) return; notify("Publication explicite créée avec identifiant opaque."); await load();
  };
  const revoke = async (id: string) => {
    ...
    if (!payload) return; notify("Publication révoquée."); await load();
  };
  return <div className={styles.grid}>
    <article className={styles.card}><h3>Publier un formulaire</h3><form className={styles.form} onSubmit={create}><label className={styles.label}>Clé du formulaire interne<input className={styles.input} value={formKey} onChange={(event) => setFormKey(event.target.value)} /></label><label className={styles.label}>Type de publication<select className={styles.select} value={kind} onChange={(event) => setKind(event.target.value)}><option value="appointment_booking">Réservation</option><option value="form">Formulaire public</option></select></label><label className={styles.label}>Champs explicitement exposés<input className={styles.input} value={fields} onChange={(event) => setFields(event.target.value)} /></label><div className={styles.row}><button className={styles.button} disabled={busy}>Publier</button><button type="button" className={styles.button} disabled={busy} onClick={() => void load()}>Actualiser</button></div></form></article>
    <article className={styles.card}><h3>Publications</h3><div className={styles.list}>{items.map((item) => <div className={styles.item} key={item.id}><strong>{item.formName} · {item.kind}</strong><div className={styles.meta}>ID public : {item.publicId}</div><div className={styles.meta}>Statut : {item.status}{item.expiresAt ? ` · expire ${formatDate(item.expiresAt)}` : ""}</div><div className={styles.row}>{item.status === "active" ? <><a className={styles.button} href={item.kind === "appointment_booking" ? `/public/booking/${item.publicId}` : `/public/forms/${item.publicId}`} target="_blank" rel="noreferrer">Ouvrir</a><button className={styles.button} disabled={busy} onClick={() => void revoke(item.id)}>Révoquer</button></> : null}</div></div>)}</div></article>
  </div>;
}
...
function InboxPanel({ request, run, busy, notify }: Shared) {
  ...
  const load = async () => { const params = new URLSearchParams(); if (query.trim()) params.set("q", query.trim()); const payload = await run(async () => request(`/api/inbox?${params}`)); if (payload) setItems(Array.isArray(payload.items) ? payload.items as Conversation[] : []); };
  const open = async (conversation: Conversation) => { const payload = await run(async () => request(`/api/inbox?conversationId=${encodeURIComponent(conversation.id)}`)); if (!payload) return; setSelected(payload.item as Conversation); setMessages(Array.isArray(payload.messages) ? payload.messages as InboxMessage[] : []); };
  const create = async (event: FormEvent) => { event.preventDefault(); const payload = await run(async () => request("/api/inbox", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ subject, relatedRecordId: relatedRecordId || null }) })); if (!payload) return; setSubject(""); notify("Conversation CRM créée."); await load(); };
  const send = async (event: FormEvent) => { event.preventDefault(); if (!selected) return; const payload = await run(async () => request("/api/inbox", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ intent: "message", conversationId: selected.id, body, direction: "internal" }) })); if (!payload) return; setBody(""); notify("Message interne ajouté."); await open(selected); };
  const markRead = async () => { if (!selected) return; const payload = await run(async () => request("/api/inbox", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ conversationId: selected.id, markRead: true }) })); if (!payload) return; notify("Conversation marquée comme lue."); await Promise.all([open(selected), load()]); };
  return <div className={styles.grid}>
    <article className={styles.card}><h3>Conversations</h3><div className={styles.row}><input className={styles.input} placeholder="Rechercher…" value={query} onChange={(event) => setQuery(event.target.value)} /><button className={styles.button} disabled={busy} onClick={() => void load()}>Rechercher</button></div><form className={styles.form} onSubmit={create}><label className={styles.label}>Objet<input className={styles.input} value={subject} onChange={(event) => setSubject(event.target.value)} required /></label><label className={styles.label}>Fiche CRM liée (optionnel)<input className={styles.input} value={relatedRecordId} onChange={(event) => setRelatedRecordId(event.target.value)} /></label><button className={styles.button} disabled={busy}>Nouvelle conversation</button></form><div className={styles.list}>{items.map((item) => <button type="button" className={styles.item} key={item.id} onClick={() => void open(item)}><strong>{item.subject}</strong><span className={styles.meta}>{item.status} · {item.unreadCount} non lu(s)</span></button>)}</div></article>
...
function QualityPanel({ request, run, busy, notify }: Shared) {
  ...
  const detect = async () => { const payload = await run(async () => request(`/api/duplicates?recordId=${encodeURIComponent(recordId)}`)); if (!payload) return; const found = Array.isArray(payload.candidates) ? payload.candidates as DuplicateCandidate[] : []; setCandidates(found); setPreview(null); notify(`${found.length} candidat(s) détecté(s), sans fusion automatique.`); };
  const previewMerge = async (candidateId: string) => { setSecondaryId(candidateId); const payload = await run(async () => request("/api/merge", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ intent: "preview", primaryId: recordId, secondaryId: candidateId }) })); if (payload) setPreview(payload.item as Record<string, unknown>); };
  const merge = async () => { if (!secondaryId || !preview || !window.confirm("Confirmer la fusion ? La fiche secondaire sera archivée et la transaction sera auditée.")) return; const payload = await run(async () => request("/api/merge", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ primaryId: recordId, secondaryId, resolution: {}, confirm: true }) })); if (!payload) return; notify("Fusion confirmée : fiche secondaire archivée, liens réaffectés et ledger créé."); setPreview(null); setSecondaryId(""); await detect(); };
  return <div className={styles.grid}><article className={styles.card}><h3>Détection explicable</h3><div className={styles.form}><label className={styles.label}>ID de la fiche principale<input className={styles.input} value={recordId} onChange={(event) => setRecordId(event.target.value)} /></label><button className={styles.button} disabled={busy || !recordId} onClick={() => void detect()}>Détecter</button></div><div className={styles.list}>{candidates.map((item) => <div className={styles.item} key={item.recordId}><strong>{item.title} · {item.classification} · {item.confidence}%</strong>{item.reasons.map((reason) => <div className={styles.reason} key={reason.criterionId}>{reason.reason} <span className={styles.meta}>({reason.compared})</span></div>)}<button className={styles.button} disabled={busy} onClick={() => void previewMerge(item.recordId)}>Comparer</button></div>)}</div></article><article className={styles.card}><h3>Fusion assistée</h3>{preview ? <><p className={styles.meta}>Principal : {recordId} · secondaire : {secondaryId}</p><pre className={styles.pre}>{JSON.stringify(preview, null, 2)}</pre><button className={styles.button} disabled={busy} onClick={() => void merge()}>Confirmer la fusion</button></> : <p className={styles.meta}>La fusion nécessite une prévisualisation puis une confirmation explicite.</p>}</article></div>;
}
...
function IntelligencePanel({ request, run, busy, notify }: Shared) {
  ...
  const inspect = async () => { const inactivityPayload = await run(async () => request(`/api/inactivity?recordId=${encodeURIComponent(recordId)}`)); if (inactivityPayload) setInactivity(inactivityPayload.item as Record<string, unknown>); const scorePayload = await run(async () => request(`/api/scoring?recordId=${encodeURIComponent(recordId)}`)); if (scorePayload) setScore(scorePayload.item as Record<string, unknown>); const nbaPayload = await run(async () => request(`/api/next-actions?recordId=${encodeURIComponent(recordId)}`)); if (nbaPayload) setRecommendations(Array.isArray(nbaPayload.recommendations) ? nbaPayload.recommendations as Recommendation[] : []); };
  const recalc = async () => { await run(async () => request("/api/inactivity", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ recordId }) })); await run(async () => request("/api/scoring", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ recordId }) })); notify("Inactivité et scoring recalculés côté serveur."); await inspect(); };
  const accept = async (item: Recommendation) => { if (!window.confirm(`Créer la tâche recommandée « ${item.action} » ?`)) return; const payload = await run(async () => request("/api/next-actions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ recordId, recommendationId: item.id }) })); if (!payload) return; notify("Action recommandée validée par l'utilisateur et matérialisée en tâche CRM."); await inspect(); };
...
function AdminPanel({ session, request, run, busy, notify }: Shared & { session: Session | null }) {
  ...
  const load = async () => { const payload = await run(async () => request("/api/configurations/v12")); if (payload) setItems(Array.isArray(payload.items) ? payload.items as ConfigItem[] : []); };
  const changeKind = (next: (typeof CONFIG_KINDS)[number]) => { setKind(next); setDefinition(JSON.stringify(defaultDefinition(next, session?.teamId ?? "TEAM_ID"), null, 2)); };
  const save = async (event: FormEvent) => {
    event.preventDefault();
    const parsed = await run(async () => {
      try { return JSON.parse(definition) as Record<string, unknown>; }
      catch { throw new Error("JSON de configuration invalide."); }
    });
    if (!parsed) return;
    if (kind === "availability" && parsed.resourceId === "TEAM_ID" && session?.teamId) parsed.resourceId = session.teamId;
    ...
    if (!payload) return; notify("Configuration v1.2 créée et versionnée."); await load();
  };
  return <div className={styles.grid}><article className={styles.card}><h3>Règles versionnées</h3><form className={styles.form} onSubmit={save}><label className={styles.label}>Type<select className={styles.select} value={kind} onChange={(event) => changeKind(event.target.value as typeof kind)}>{CONFIG_KINDS.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label className={styles.label}>Nom<input className={styles.input} value={name} onChange={(event) => setName(event.target.value)} /></label><label className={styles.label}>Définition<textarea className={styles.textarea} value={definition} onChange={(event) => setDefinition(event.target.value)} /></label><div className={styles.row}><button className={styles.button} disabled={busy || session?.role !== "admin"}>Créer la règle</button><button type="button" className={styles.button} disabled={busy} onClick={() => void load()}>Actualiser</button></div></form><p className={styles.meta}>Les seuils et poids sont des paramètres administratifs de triage, pas des valeurs métier imposées par le code.</p></article><article className={styles.card}><h3>Configurations v1.2</h3><div className={styles.list}>{items.map((item) => <div className={styles.item} key={item.id}><strong>{item.name}</strong><div className={styles.meta}>{item.kind} · v{item.version} · {item.active ? "active" : "inactive"}</div><details><summary>Définition</summary><pre className={styles.pre}>{JSON.stringify(item.definition, null, 2)}</pre></details></div>)}</div></article></div>;
}
...
function toLocalInput(date: Date) { const shifted = new Date(date.getTime() - date.getTimezoneOffset() * 60_000); return shifted.toISOString().slice(0, 16); }
